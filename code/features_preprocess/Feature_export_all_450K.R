...
all_sites_winid<-read.table(paste(home,dataset,'selected_450k_pos_winid.csv',sep=''),header=FALSE)
...
for (file in files){
  ...
  write.csv(all,file=paste(dir,dataset,file_name,'_all_450k.csv',sep=''),row.names=F)
  #rm(readmat,pos,neg)
  rm(readmat,all)
}
