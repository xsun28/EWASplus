...
def RNASeq_Preprocessing(file):
    ...
    if not os.path.exists(data_dir+single_file+'.bed'):
        logger.info('start downloading RNASeq {} bam file and convert to bed file'.format(file))
        subprocess.check_call([script_path,file,data_dir])
        logger.info('finish downloading RNASeq {} bam file and convert to bed file'.format(file))
    else:
        logger.info('RNASeq '+single_file+'.bed already exists')
    if os.path.exists(RNASeq_h5s+single_file):
        logger.info('Binned RNASeq '+single_file+" already exists, skipping")
    else:
        logger.info("binning RNASeq "+single_file+" ...")
    ...
    if os.path.exists(data_dir+single_file+'.bam'):
        ...
        logger.info('RNASeq {}.bam is removed'.format(single_file))
    #os.remove(data_dir+single_file+'.bed')


def process_meta_info_table(meta_info_table,downloaded_files):
    logger.info('downloaded RNASeq fies are: '+str(downloaded_files))
    ...
    for ind, row in selected_meta_data_df.iterrows():
        if row['File accession'] in downloaded_files:
            ...
            logger.info('Adding file {} to experiment accession {}'.format(row['File accession'],row['Experiment accession']))
...
logger.info('combine RNASeq features by expriments')
exp_dir = home+'data/RNASeq/'
h5s_dir = home+'data/RNASeq/single_file/'
logger.info('combined experiment output dir: '+exp_dir)
...
logger.info('RNASeq experiment file dictionary: {}'.format(file_exp_dict))
for exp_key in file_exp_dict:
    logger.info("Combining files of experiment %s" % exp_key)
    if os.path.exists(os.path.join(exp_dir, exp_key)):
        logger.info('experiment: '+exp_key+' already exists, skipping...')
        continue

    file_list = file_exp_dict[exp_key]
    files_count = len(file_list)
    logger.info("there are %i files for this exp" % files_count)
    for file_ind in range(files_count):
        logger.info("Processing {} file".format(file_list[file_ind]))
        if (file_ind==0):
    ...
    with pd.HDFStore(os.path.join(exp_dir, exp_key),'w') as h5s:
        h5s[exp_key] = res_df   
        logger.info('save experiment level RNASeq data to {}'.format(os.path.join(exp_dir, exp_key)))
