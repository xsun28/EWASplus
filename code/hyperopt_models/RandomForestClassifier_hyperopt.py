...
def rf_loss(params):
    ...
    score = cal_opt_score(rf_estimator,weight_factor)
    estimators.extend([rf_estimator])
...
rf_param = {'n_estimators': 10+hp.randint('n_estimators',500),'max_depth':3+hp.randint('max_depth',30), 'min_samples_split': 3+hp.randint('min_samples_split',30),'min_samples_leaf': 1+hp.randint('min_samples_leaf',5),'weight_factor':weight_factor}
if class_num <= 2:
...
trial = Trials()
estimators = []
best = fmin(rf_loss,rf_param,algo=tpe.suggest,max_evals=max_iter,trials=trial)
best_ix = np.argmin(trial.losses())
best_estimator = estimators[best_ix]
best_params = best_estimator.get_params()
best_params['weight_factor'] = best['weight_factor']
joblib.dump(best_params,model_dir+'RandomForestClassifier.pkl')